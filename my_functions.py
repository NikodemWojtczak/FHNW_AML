...
class HelperFunctions:
    ...
    def calculate_birthday(self, df):
        ...
        for birth_number in df["birth_number"]:
            ...
            age = (
                base_date.year
                - birth_day.year
                - ((base_date.month, base_date.day) < (birth_day.month, birth_day.day))
            )
    ...
    def get_transactions_within_window(
        self, trans_disp, client_id, start_date, end_date
    ):
        client_trans = trans_disp[trans_disp["client_id"] == client_id]
        window_trans = client_trans[
            (client_trans["trans_date"] >= start_date)
            & (client_trans["trans_date"] <= end_date)
        ]
        return window_trans
    ...
    def calculate_wealth_metrics(self, transactions):
        if transactions.empty:
            return pd.Series({"avg_balance": 0, "balance_std": 0, "ending_balance": 0})
        else:
            balances = transactions["balance"]
            avg_balance = balances.mean()
            balance_std = balances.std()
            ending_balance = transactions.sort_values("trans_date", ascending=True)[
                "balance"
            ].iloc[-1]
            return pd.Series(
                {
    ...
    def calculate_transaction_metrics(self, transactions):
        if transactions.empty:
            ...
            num_debit_transactions = len(
                transactions[transactions["trans_type"] == "withdrawal"]
            )
            num_credit_transactions = len(
                transactions[transactions["trans_type"] == "credit"]
            )
            total_debit_amount = transactions[
                transactions["trans_type"] == "withdrawal"
            ]["trans_amount"].sum()
            total_credit_amount = transactions[transactions["trans_type"] == "credit"][
                "trans_amount"
            ].sum()
            avg_transaction_amount = transactions["trans_amount"].mean()
            num_days = (
                transactions["trans_date"].max() - transactions["trans_date"].min()
            ).days + 1
    ...
    def tsfresh_features(self, transactions):
        ...
        return extracted_features.iloc[0]

    def plot_monthly_trends(self, trans_disp, account_id):

        account_data = trans_disp[trans_disp["account_id"] == account_id].copy()

        account_data["trans_date"] = pd.to_datetime(
            account_data["trans_date"], errors="coerce"
        )

        account_data["year_month"] = account_data["trans_date"].dt.to_period("M")

        monthly_stats = (
            account_data.groupby("year_month")
            .agg(avg_balance=("balance", "mean"), total_amount=("trans_amount", "sum"))
            .reset_index()
        )
    ...
    def evaluate_model_with_top_features(
        self,
        y_train_exp,
        importance_df,
        X_train_exp,
    ...
    def plot_gender_distribiution(self, client):
        ...
        axes[0].pie(
            gender_counts, labels=gender_counts.index, autopct="%1.1f%%", colors=colors
        )
        axes[0].set_title("Distribution of Gender")

        axes[1].boxplot(
            [
                client[client["gender"] == "male"]["age"],
                client[client["gender"] == "female"]["age"],
            ],
            labels=["Men", "Women"],
            patch_artist=True,
    ...
    def plot_average_salary_distribiution(self, district):
        ...
        plt.bar(
            district["district_name"], district["average_salary"], color="forestgreen"
        )
        plt.xlabel("District")
        ...
        plt.figure(figsize=(12, 6))
        plt.scatter(
            district["inhabitants"], district["average_salary"], color="forestgreen"
        )
    ...
    def plot_transaction_info(self, trans):
        ...
        sns.countplot(
            data=trans,
            y="operation",
            order=trans["operation"].value_counts().index,
            color="forestgreen",
        )
        ...
        transaction_counts = (
            trans.groupby("transaction_type")["trans_id"]
            .count()
        ...
        axes[0, 0].set_xticks([])
        bars = axes[0, 1].bar(
            transaction_counts["transaction_type"],
            transaction_counts["transaction_count"],
            color="forestgreen",
        )
        ...
        accounts_per_month = trans.groupby("first_transaction_year_month")[
            "account_id"
        ].nunique()
    ...
    def plot_loan_info(seld, loan):
        ...
        plt.bar(
            duration_counts_sorted.index.astype(str),
            duration_counts_sorted.values,
            color="forestgreen",
        )
    ...
    def plot_correlation_new_accounts_cards(self, card_clients):
        loans_per_month = card_clients.groupby("loan_taken_per_month")[
            "account_id"
        ].nunique()
    ...
    def plot_roc_curves(
        ...
        plt.plot(
            fpr,
            tpr,
            label="Baseline Logistic Regression (AUC = {:.2f})".format(
        ...
        plt.plot(
            fpr_exp,
            tpr_exp,
            label="Expanded Logistic Regression (AUC = {:.2f})".format(
                expanded_metrics["ROC AUC"]
            ),
        )
        ...
        plt.plot(
            fpr_rf,
            tpr_rf,
            label="Random Forest (AUC = {:.2f})".format(rf_metrics["ROC AUC"]),
        )
        ...
        plt.plot(
            fpr_opt_rf,
            tpr_opt_rf,
            label="Optimized Random Forest (AUC = {:.2f})".format(
                opt_rf_metrics["ROC AUC"]
            ),
        )
